...
# INDEX_PATH = os.path.normpath(os.path.dirname(os.path.abspath(__file__)))
# ENV_PATH = os.path.normpath(f"{str(Path(INDEX_PATH).parent.parent)}/.env")
# def _get_env(key: str, dotenv_path: str = None):
#     load_dotenv(dotenv_path=dotenv_path, override=True)
#     return os.getenv(key=key, default=None)


# def get_env(key, _raise=False, _default_value=None):
#     dot_env_path = os.environ.get("DOT_ENV_PATH")
#     if dot_env_path is None:
#         dot_env_path=ENV_PATH
#     _r = _get_env(key=key, dotenv_path=dot_env_path)
#     if _r is None and _raise:
#         raise Exception(f"CHECK EXISTENCE OF '{key}'")
#     if _r is None and not _raise:
#         print(f"USING default_value of '{key}'")
#         return _default_value
#     return _r

# def parallel_executor(func: any, elements: list, max_workers=None, verbose=0) -> list:
#     info_list = []
#     with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:

#         for i, element in enumerate(elements):
#             if verbose >= 1:
#                 print(f"getting {i + 1:04d} from {len(elements)} | {element}")
#             info_list.append(executor.submit(
#                 func,
#                 element,
#             ))
#         r_list = []
#         for r in concurrent.futures.as_completed(info_list):
#             r_list.append(r.result())
#     return r_list


# def sequential_executor(func: any, elements: list, verbose=0) -> list:
#     info_list = []
#     for i, element in enumerate(elements):
#         if verbose >= 1:
#             print(f"getting {i + 1:04d} from {len(elements)} | {element}")
#         info_list.append(func(element))
#     return info_list
...
def obtener_datos_cosmos_icmm(app):
    cache = app.cache
...
#     # Intentamos obtener datos de la caché para Antucoya
#     df_ph_ant = cache.get('ph_ant_data_icmm')
#     if df_ph_ant is None:
#         # Si no están en caché, obtenemos los datos desde CosmosDB
#         cosmos_client = app.config['COSMOS_CLIENT']
#         df_ph_ant = cosmos_client.get_items_from_container(
#             'PH_ANT', 
#             'amsa_datos_hidricos_web.dmt_ant_reporte_hidrico_icmm'
...
#     # Intentamos obtener datos de la caché para Los Pelambres
#     df_ph_mlp = cache.get('ph_mlp_data_icmm')
#     if df_ph_mlp is None:
#         cosmos_client = app.config['COSMOS_CLIENT']
#         df_ph_mlp = cosmos_client.get_items_from_container(
...
#     # Intentamos obtener datos de la caché para Zaldivar
#     df_ph_cmz = cache.get('ph_cmz_data_icmm')
#     if df_ph_cmz is None:
#         cosmos_client = app.config['COSMOS_CLIENT']
#         df_ph_cmz = cosmos_client.get_items_from_container(
...
#     # Guardamos la combinación en la caché para su uso futuro
#     cache.set('ph_data_icmm', df_combined, timeout=3600)

#     return df_combined

def generar_saludo():
    greetings = [
        "Es genial tenerte aquí",  # Lunes
        "Buen día",  # Martes
        "Bienvenid@",  # Miércoles
        "Hola",  # Jueves
        "Feliz viernes",  # Viernes
        "Buen fin de semana",  # Sábado
        "Descansa hoy",  # Domingo
    ]

    # Determina el saludo basado en el día de la semana
    day_of_week = datetime.now().weekday()
    greeting = greetings[day_of_week]

    try:
        user, email = get_microsoft_user_info()
        primer_nombre = user.split()[0]
        return f"¡{greeting}, {primer_nombre}!"
...
def get_href_mapping():
    return {
        'Antucoya': '/diagramaflujoant',
        'Centinela Oxido': '/diagramaflujocenox',
        'Centinela Sulfuro': '/diagramaflujocensul',
...
def procesar_datos_icmm(app, compania, mes, anio, mtd_ytd_switch):
...
def procesar_datos_icmm_delta_perc(app, compania, mes, anio, mtd_ytd_switch):
    if not mes or not anio:
